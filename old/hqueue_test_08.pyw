...
def upd(model1,model2):
    ...
    thread_upd.finished.connect(lambda: thread_upd.initChildrenWindow)
...
def clickedCell(fin=True):
    thread = worker(None,True)
    modelChildren=jobsTable(hq_server,False,False)

    if fin:
        ix = table_fin.selectedIndexes()[0].row()
        parentId = modelFin.arraydata[ix][0]
        print modelChildren.getChildren(parentId)
    else:
        ix = table_unfin.selectedIndexes()[0].row()
        parentId = modelFin.arraydata[ix][0]
        print modelChildren.getChildren(parentId)
...
if __name__ == '__main__':
    ...
    hou_icon = QtGui.QIcon("/studio/tools/icons/hou.png")
    mainWindow.setWindowIcon(hou_icon)
    ...
    tabs_widget.setTabEnabled(1,False)
    tabs_widget.setTabToolTip (200, 'This tab contains submitted jobs')
    tabs_widget.setTabToolTip (1, 'This tab contains children jobs, selected by double-click at any submitted job')
    ...
    upd_button = QtGui.QPushButton('Update', tab_main_jobs)
    upd_button.clicked.connect(lambda: upd(modelRun,modelFin))
    upd_button.setMaximumWidth(500)

    sb = QtGui.QStatusBar(mainWindow)
    sb.setStyleSheet('QStatusBar { background-color: rgb(255, 255, 255); border:1px solid rgb(170, 170, 170); }')
